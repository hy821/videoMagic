...
@property (nonatomic,strong) SDCollectionViewCell *currentCell;
...
-(void)scrolToIndex:(NSInteger)index complete:(void (^)(SDCollectionViewCell *))complete
{
      if (0 == _totalItemsCount) return;
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.01 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if(complete)
        {
            complete((SDCollectionViewCell*)[_mainView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:index inSection:0]]);
        }
    });
}
- (void)setupTimer
...
- (void)layoutSubviews
{
    ...
    //HyAdd
    if (self.pageControlAliment == SDCycleScrollViewPageContolAlimentHomePage) {
        y = self.mainView.sd_height - size.height - 10 - self.sizeH(20);
    }
...
//    cell.model = self.recycleDataArr[itemIndex];
    
    return cell;
}

- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
//    SDCollectionViewCell *theCell = (SDCollectionViewCell *)cell;
//    long itemIndex = indexPath.item % self.imagesGroup.count;
//    RecycleModel *m = self.recycleDataArr[itemIndex];
//    if (m.ad && m.ad.sspId && m.ad.sspType) {
//        self.currentCell = theCell;
//    }else {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.delegate respondsToSelector:@selector(cycleScrollView:didSelectItemAtIndex:cell:)]) {
        [self.delegate cycleScrollView:self didSelectItemAtIndex:indexPath.item % self.imagesGroup.count cell:(SDCollectionViewCell*)[collectionView cellForItemAtIndexPath:indexPath]];
    }
}
...
//    if (self.tabName) {
//        NSDictionary *dic = @{TAB_NAME : self.tabName};
//        [[BuryingPointManager shareManager] buryingPointWithEventID:BP_HomeCycleScroll andParameters:dic];
//    }
}
