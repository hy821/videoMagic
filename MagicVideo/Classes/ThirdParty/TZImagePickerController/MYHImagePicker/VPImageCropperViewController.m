...
- (id)initWithImage:(id)originalImage cropFrame:(CGRect)cropFrame limitScaleRatio:(NSInteger)limitRatio {
    self = [super init];
    if (self) {
        self.isShow = NO;
        self.cropFrame = cropFrame;
        self.limitRatio = limitRatio;
        self.originData = originalImage;
...
- (void)initView {
   ...
   }else
   {
       ...
       self.ratioView = [[UIView alloc] initWithFrame:CGRectZero];
...
-(void)resolveImg{
    ...
    TZAssetModel * model = self.originData;
    WS()
    [manager getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        
        dispatch_async(dispatch_get_main_queue(), ^{
        weakSelf.originalImage = [weakSelf fixOrientation:photo];
        [weakSelf.showImgView setImage:self.originalImage];
        // scale to fit the screen
        CGFloat oriWidth = weakSelf.cropFrame.size.width;
        CGFloat oriHeight = weakSelf.originalImage.size.height * (oriWidth / self.originalImage.size.width);
        CGFloat oriX = weakSelf.cropFrame.origin.x + (self.cropFrame.size.width - oriWidth) / 2;
        CGFloat oriY = weakSelf.cropFrame.origin.y + (self.cropFrame.size.height - oriHeight) / 2;
        weakSelf.oldFrame = CGRectMake(oriX, oriY, oriWidth, oriHeight);
        weakSelf.latestFrame = self.oldFrame;
        weakSelf.showImgView.frame = self.oldFrame;
        weakSelf.largeFrame = CGRectMake(0, 0, self.limitRatio * self.oldFrame.size.width, self.limitRatio * self.oldFrame.size.height);
        weakSelf.ratioView.frame = self.cropFrame;
    });
    }];
}
- (void)initControlBtn {
    UIButton *cancelBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - 50.0f, 100, 50)];
    cancelBtn.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.3];
    ...
    confirmBtn.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.3];
