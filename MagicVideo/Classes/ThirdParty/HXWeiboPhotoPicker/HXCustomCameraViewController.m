...
//        self.navigationItem.rightBarButtonItems = @[rightBtn1,rightBtn2];
        ...
        [self.flashBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.size.mas_equalTo(_flashBtn.currentImage.size);
        }];
        [self.changeCameraBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.right.equalTo(self.flashBtn.mas_left).offset(-10.f);
            make.centerY.equalTo(self.flashBtn);
        }];
    }else {
...
- (HXFullScreenCameraPlayView *)playView {
    if (!_playView) {
        _playView = [[HXFullScreenCameraPlayView alloc] initWithFrame:CGRectMake(0, 0, 70, 70) color:DarkGray_Color];
