...
- (void)viewMoveNextWithType:(WXSTransitionAnimationType )type andContext:(id<UIViewControllerContextTransitioning>)transitionContext{
    ...
    if([self.startView isKindOfClass:[UIImageView class]])
    {
        UIImageView * imageV = [[UIImageView alloc]initWithImage: ((UIImageView*)self.startView).image];
        imageV.frame = self.startView.frame;
        startView = imageV;
    }else
    {
      startView =  [self.startView snapshotViewAfterScreenUpdates:NO];
    }
    startView.contentMode = self.startView.contentMode;
    startView.clipsToBounds = self.startView.clipsToBounds;
    UIView *containerView = [transitionContext containerView];
    ...
    if (type == WXSTransitionAnimationTypeViewMoveToNextVC) {
        ...
        [UIView animateWithDuration:self.animationTime delay:0.0 usingSpringWithDamping:0.7 initialSpringVelocity:0 options:0 animations:^{
            AnimationBlock();
