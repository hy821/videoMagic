...
//HyAdd  提示未安装时,知道是哪个App
@property (nonatomic,assign) UMSocialPlatformType sharePlatformType;
                       ...
                       viewController:(UIViewController *)viewController
{
    ...
    [[SDWebImageManager sharedManager] loadImageWithURL:[NSURL URLWithString:imageUrl] options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
        ...
        if(image == nil)
        {
            image = Image_Named(@"AppIcon");
        }
        SSDissMissGifHud(MainWindow, nil);
        /*! 创建网页内容对象 */
...
- (void)alertWithError:(NSError *)error
{
    ...
    if (!error) {
        ...
        SSSuccessOrErrorToast(result, MainWindow, YES);
    }
    else{
        ...
        {
            SSMBToast(@"分享已取消", MainWindow);
        }else{
            if (error.code == 2008) {
                if (self.sharePlatformType == UMSocialPlatformType_WechatSession || self.sharePlatformType == UMSocialPlatformType_WechatTimeLine) {
                    SSSuccessOrErrorToast(@"您未安装微信", MainWindow, NO);
                }else if (self.sharePlatformType == UMSocialPlatformType_QQ || self.sharePlatformType == UMSocialPlatformType_Qzone) {
                    SSSuccessOrErrorToast(@"您未安装QQ", MainWindow, NO);
                }
            }else {
                SSSuccessOrErrorToast(error.localizedDescription, MainWindow, NO);
            }
        }
    }
}
             ...
             loginCallback:(BAUMLoginCallback)loginCallback
{
    ...
    if (message) {
        ...
        .LeeTitle(@"提示")
        .LeeContent(message).LeeAddAction(^(LEEAction *action) {
            
            action.type = LEEActionTypeDefault;
            
            action.title = @"知道了";
            action.font = KFONT(14);
            action.titleColor = Black_Color;
            
            action.highlightColor = Black_Color;
...
#pragma mark 微信分享_纯图片
- (void)hy_wechatShareWithShareType:(BAUM_SHARE_TYPE)shareType
                     viewController:(UIViewController *)viewController {
    switch (shareType) {
        case BAUM_SHARE_TYPE_IMAGE:
            [self hy_shareImageToPlatformType:UMSocialPlatformType_WechatSession
                                   thumbImage:nil
                                     bigImage:_screenShotsImage
                               viewController:viewController];
            break;
        default:
            break;
...
#pragma mark 微信朋友圈分享_纯图片
- (void)hy_wechatTimeLineShareWithShareType:(BAUM_SHARE_TYPE)shareType
                             viewController:(UIViewController *)viewController {
    switch (shareType) {
        case BAUM_SHARE_TYPE_IMAGE:
            [self hy_shareImageToPlatformType:UMSocialPlatformType_WechatTimeLine
                                   thumbImage:nil
                                     bigImage:_screenShotsImage
                               viewController:viewController];
            break;
...
#pragma mark 分享纯图片
- (void)hy_shareImageToPlatformType:(UMSocialPlatformType)platformType
                         thumbImage:(NSString *)thumbImage
                     ...
                     viewController:(UIViewController *)viewController
{
    ...
    [shareObject setShareImage:bigImage];
