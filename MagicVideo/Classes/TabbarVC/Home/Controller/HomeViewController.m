...
- (NSMutableArray *)dataSource {
    if (!_dataSource) {
        ...
        NSArray *a = @[URL(TmpVideoUrl),URL(TmpVideoUrl),URL(TmpVideoUrl),URL(TmpVideoUrl),URL(TmpVideoUrl),URL(TmpVideoUrl)];
        [_dataSource addObjectsFromArray:a];
...
- (void)zf_playTheVideoAtIndexPath:(NSIndexPath *)indexPath {
    //点击cell上的播放按钮, 和didSelectRowAtIndexPath类似,只是没跳详情页
...
//        @strongify(self)
        ...
        //    [[SSPlayer manager].controlView showTitle:model.name
        //                 coverURLString:model.poster.url
        //                 fullScreenMode:model.isVerticalVideo ? ZFFullScreenModePortrait : ZFFullScreenModeLandscape];
        
        [[SSPlayer manager].controlView showTitle:@"一部好莱坞顶级科幻动作电影 简直是一场视觉的饕餮盛宴 极度震撼"
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    vc.vcType = VideoDetailType_WithPlayer;
    //        vc.model = mDetail;
...
- (void)dealloc {
    ...
    SSLog(@"  HomeViewController--->>> Dealloc");
}
